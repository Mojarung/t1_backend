...
class ResumeAnalysisService:
    ...
    async def analyze_resume_with_ai(self, job_description: str, resume_text: str) -> Dict[str, Any]:
        ...
        prompt = (
            ...
            "- Включай в анализ ТОЛЬКО компании, которые действительно существуют и являются релевантными для индустрии.\n"
            "- Исключай подозрительные или вымышленные названия компаний (например, 'ООО Рога и Копыта', 'ИП Иванов', 'Компания X' без дополнительных деталей).\n"
            "- Приоритет отдавай известным IT-компаниям, стартапам с четким описанием деятельности, международным корпорациям.\n"
            "- Если компания незнакома, но имеет подробное описание деятельности и выглядит реалистично - включай в анализ.\n\n"
            f"**ВАКАНСИЯ:**\n{job_description}\n\n"
            f"**РЕЗЮМЕ:**\n{resume_text}\n\n"
    ...
    async def analyze_resume_for_profile(self, resume_text: str) -> Dict[str, Any]:
        ...
        prompt = (
            ...
            "ВАЖНО: Внимательно извлекай все навыки, технологии, инструменты и компетенции, упомянутые в резюме.\n\n"
            f"**РЕЗЮМЕ:**\n{resume_text}\n\n"
            "Верни результат в следующем JSON формате:\n"
            "{\n"
            "  \"personal_info\": {\n"
            ...
            "    ],\n"
            "    \"other_competencies\": [\"Git\", \"Docker\", \"AWS\", \"PostgreSQL\", \"REST API\", \"Микросервисы\"]\n"
            "  },\n"
            "  \n"
            "ИНСТРУКЦИИ ПО ИЗВЛЕЧЕНИЮ НАВЫКОВ:\n"
            "- programming_languages: языки программирования (Python, Java, JavaScript, C++, Go, Rust и т.д.)\n"
            "- foreign_languages: иностранные языки с указанием уровня\n"
            "- other_competencies: все остальные технологии, инструменты, фреймворки, библиотеки, системы (Git, Docker, Kubernetes, PostgreSQL, MongoDB, Redis, REST API, GraphQL, микросервисы, CI/CD, Linux, Windows, Agile, Scrum и т.д.)\n"
            "\n"
            "  \"experience\": [\n"
            "    {\n"
            "      \"role\": \"Должность\",\n"
